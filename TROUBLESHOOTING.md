...
## 13. 건강 불안 등 점수가 엉뚱하게 높음(95) → 단편 STT 입력 문제

- **증상**: 허리 통증·스트레스를 호소했는데 `건강 불안 95`(=건강함)로 표시. 점수 체계가 반대로 매겨진 것 아닌지 의심.
- **검증**: 점수 체계는 **높을수록 건강(좋음), 낮을수록 나쁨**이 맞고 모델도 그대로 채점함. 동일 텍스트를 깨끗하게 넣으면 로컬 Gemini·원격 ADK 모두 `health_anxiety 65 / daily_vitality 45` 등 합리적 값 반환(반전 아님).
- **원인**: STT 가 발화 일부만 잡음. job 로그 `[stt] real transcription (… 오디오 2.5s): '몸'` 처럼 **단편·잡음 텍스트**가 LLM 에 들어가면 점수가 5~90 으로 들쭉날쭉(내용 없는 입력 → 환각). 95 는 그렇게 저장된 값.
  - 단편의 원인: 미들웨어 VAD(`internal/vad/detector.go`) RMS 임계 100 이 마이크(autoGainControl=off, 부드러운 발화)에 비해 높아 문장을 일찍 끊음.
- **해결**:
  1. **분석 가드**(`pipeline/analysis_service.py` `_meaningful_stt`): 한글 음절 4개 미만 또는 1~2음절 토큰 반복('어 어 어')이면 5지표 분석 **생략(점수 미저장)** → 엉터리 점수가 대시보드에 안 남음. 로그 `[SLOW/SKIP]`.
  2. **동적 요약**(`_build_summary`): 하드코딩이던 "건강 불안·사회적 고립 신호 관찰됨"(점수와 모순)을 **실제 점수<70 지표만** 나열하도록 교체.
  3. **VAD 튜닝 가능화**(`detector.go`): `VAD_RMS_THRESHOLD`(기본100), `VAD_SILENCE_MS`(기본1500) 환경변수화. 발화가 잘리면 미들웨어 로그 `VAD rms=` 를 보고 말할 때 값보다 약간 낮게 임계를 내린다(예: `VAD_RMS_THRESHOLD=40`).
- **교훈**: "이상한 점수"는 모델/방향 문제가 아니라 **입력(STT) 품질** 문제일 때가 많다. 분석 전 입력 유효성 검사 + 점수 기반 요약으로 신뢰성 확보.

---

## 14. 점수가 대화(스트레스)와 안 맞게 높음 + '전체보기' 미동작

- **증상**: 허리 통증·스트레스 대화인데 건강불안 98·인지부하 95 등 높게 표시. "AI 답변 텍스트까지 채점에 반영되는 것 같다". 대화기록 '전체보기' 드롭다운 클릭해도 안 펼쳐짐.
- **전체보기 (프론트 버그)**: `[id]/page.tsx`에서 ① 데이터 빌더가 `chatLog.slice(0,4)`로 **잘라서** 전달 → 숨긴 메시지가 클라에 아예 없음, ② 버튼에 `onClick` 없음. → 전체 chatLog 전달 + `chatExpanded` 상태/토글 추가(접기 ▲ 포함).
- **점수 (오해 + 진짜 원인)**:
  - 분석 입력엔 **AI 답변 텍스트가 들어가지 않음**(검증: `text`=어르신 STT, `history`=고정 인사말+어르신 발화, `emotion_6`=음성감정). 감정 라벨 매핑도 정상(`config.id2label == LABELS`, MATCH=True).
  - 진짜 부풀림 원인: **음성감정 오분류**. 스트레스 음성을 감정모델이 '기쁨/놀라움'으로 노이즈 분류 → 긍정 감정이라 LLM이 점수를 올림(기쁨 emotion→health 88 vs 슬픔→75).
  - **교정(원격 재배포 없이)**: `analysis_service._augment_emotion()` — 음성감정 페이로드에 "발화 '내용'을 1순위로 판단, 감정과 상충하면 내용 우선, AI 답변은 채점 근거 아님" 지침을 주입. `build_data_prompt`가 이 JSON을 그대로 프롬프트에 넣으므로 **원격 ADK·로컬 둘 다 반영**.
  - **남는 한계(=점수표 의미)**: 5지표는 특정 임상 차원이라 "건강 얘기=건강불안 낮음"이 아님. health_anxiety는 **건강염려증**(질병 공포) 척도라 단순 통증 호소론 높게 유지됨. 인지부하 95는 **또렷이 말함→인지 정상**이라 오히려 맞음. 스트레스는 일상활력↓·감정변동에 주로 반영됨.
- **데모 팁**: 특정 지표를 '주의'로 보이려면 그에 맞는 발화를 해야 함(`_demo_scripts.py` 시나리오 참고) — 건강불안↓="큰 병 걸린 것 같아 무서워요", 사회적고립↓="아무도 안 찾아와 외로워요", 인지부하↓="자꾸 깜빡해요".
- **적용**: analysis_service 재기동 필요(가드·요약·감정지침 모두 반영). 프론트는 dev 핫리로드.
